...
namespace AtelierXNA
{
    ...
    public class AfficheurTexte : Cadre
    {
        ...
        void EncoderMessage()
        {
            ...
            //{
            //    MessageListInt.Add(EncodeurFont.EncoderASCIIEnFont(b));//On prend les char en ASCII, on met les numéros des caractères (dec) pour qu'y soient équivalents aux cases de la font.
            //}

            MessageListInt = EncodeurFont.ConvertirStringEnListeInt(Message);

            if (NBDeCaractèresParLigne > 5 && MessageListInt.Count > NBDeCaractèresParLigne)//Si on a plus qu'une ligne de texte
                ModifierPourPasCouperDeMot(MessageListInt);

            //TrimDébutToutesLesLignes();
            //ModifierPourPasCouperDeMot();
        }
